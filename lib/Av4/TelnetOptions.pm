...
use Av4::Telnet qw/
  ...
  _256col
  /;
use constant { DEBUGTELNETOPTS => 0, };

use Class::XSAccessor {
    constructor => '_new',
    accessors   => [
        qw/user mccp terminaltype naws_w naws_h
...
sub send_data {
    ...
    my $outdeflate;
    my $deflatestatus = $self->zstream->deflate( $out, \$outdeflate );
    ...
    my $outflush;
    my $flushstatus = $self->zstream->flush( $outflush, Z_SYNC_FLUSH );
...
sub analyze {
    ...
    foreach my $char ( split( '', $data ) ) {
        $log->debug( "$charno) Got character: ", "dec ", sprintf( "%d", ord $char ), " hex ", sprintf( "%x", ord $char ), ' ' ) if DEBUGTELNETOPTS;
        $charno++;
        if ( $self->state_iac == 0 ) {    # only IAC or standard characters allowed
            ...
            if ( defined $TELOPTIONS{ ord $char } ) {
                ...
                if ( $self->state_do == 1 ) {
                    ...
                    if ( ord $char == TELOPT_TTYPE ) {
                        ...
                        $self->user->print_raw( sprintf( "%c%c%c%c%c%c", TELOPT_IAC, TELOPT_SB, TELOPT_TTYPE, 1, TELOPT_IAC, TELOPT_SE, ) );
            ...
            if ( $self->state_sb == 2 ) {    # Got IAC SB OPTION, waiting on DATA
                ...
                if ( ord $char == TELOPT_SE ) {
                    ...
                    warn( "Client ", $self->user, ": shouldn't have received this (IAC=1, <240)" );
